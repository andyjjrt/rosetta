...
- Prefer executable config over the README. Runtime uses `lava-lyra`, not Pomice; Lavalink is the default media backend, while NodeLink has a separate alternative Compose file.
...
- Local media-node startup is `docker compose -f docker-compose.dev.yaml up -d`. It starts two Lavalink nodes on 2333 and 2334; use `docker-compose.nodelink.yaml` for the two-node NodeLink alternative. `LAVALINK_LOCAL_NODE_COUNT` controls sequential local endpoints and production Compose sets it to one.
